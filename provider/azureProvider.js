...
class AzureProvider {
  ...
  Login() {
    return login({
      ...
      .catch(error => {
        error.message =
          error.message ||
          (error.body ? error.body.message : "Failed logging in to Azure");
        throw error;
      });
  }
  ...
  CreateFunctionApp() {
    ...
    if (gitUrl) {
      templateFilePath = path.join(
        __dirname,
        "armTemplates",
        "azuredeployWithGit.json"
      );
    }
    if (this.serverless.service.provider.armTemplate) {
      templateFilePath = path.join(
        this.serverless.config.servicePath,
        this.serverless.service.provider.armTemplate.file
      );
      const userParameters = this.serverless.service.provider.armTemplate
        .parameters;
      const userParametersKeys = Object.keys(userParameters);

      for (
        let paramIndex = 0;
        paramIndex < userParametersKeys.length;
        paramIndex++
      ) {
        const item = {};

        item[userParametersKeys[paramIndex]] = {
          value: userParameters[userParametersKeys[paramIndex]]
        };
    ...
    if (environmentVariables) {
      const appSettingsPath =
        '$.resources[?(@.kind=="functionapp")].properties.siteConfig.appSettings';
  ...
  DeleteResourceGroup() {
    ...
    return new BbPromise((resolve, reject) => {
      resourceClient.resourceGroups.deleteMethod(
        resourceGroupName,
        (error, result) => {
          if (error) {
            reject(error);
  ...
  getAdminKey() {
    const options = {
      url: `https://${functionAppName}${config.scmDomain}${
        config.masterKeyApiPath
      }`,
      json: true,
      headers: {
        Authorization:
          config.bearer +
          principalCredentials.tokenCache._entries[0].accessToken
      }
    };
  ...
  pingHostStatus(functionName) {
    const requestUrl = `https://${functionAppName}${
      config.functionAppDomain
    }/admin/functions/${functionName}/status`;
    const options = {
  ...
  getDeployedFunctionsNames() {
    const requestUrl = `https://${functionAppName}${config.scmDomain}${
      config.functionsApiPath
    }`;
    const options = {
    ...
    return new BbPromise((resolve, reject) => {
      if (existingFunctionApp) {
        this.serverless.cli.log(
          "Looking for deployed functions that are not part of the current deployment..."
        );
        request(options, (err, res, body) => {
          if (err) {
            if (err.message.includes("ENOTFOUND")) {
            ...
            if (res.statusCode === 200) {
              for (
                let functionNamesIndex = 0;
                functionNamesIndex < body.length;
                functionNamesIndex++
              ) {
  ...
  getInvocationId(functionName) {
    const options = {
      url: `https://${functionAppName}${
        config.scmDomain
      }${config.logInvocationsApiPath +
        functionAppName}-${functionName}/invocations?limit=5`,
      method: "GET",
      json: true,
      headers: {
        Authorization:
  ...
  invoke(functionName, eventType, eventData) {
    if (eventType === "http") {
        ...
        if (typeof eventData === "string") {
          try {
            ...
            return BbPromise.reject(
              "The specified input data isn't a valid JSON string. " +
      ...
      return new BbPromise((resolve, reject) => {
        const options = {
          ...
          url: `http://${functionAppName}${
            config.functionAppDomain
          }${config.functionAppApiPath + functionName}?${queryString}`,
          method: "GET",
          json: true
        };
    ...
    return new BbPromise((resolve, reject) => {
      request(options, (err, res) => {
        ...
        this.serverless.cli.log(
          `Invoked function at: ${requestUrl}. \nResponse statuscode: ${
            res.statusCode
          }`
        );
        resolve(res);
  ...
  syncTriggers() {
    ...
    return new BbPromise((resolve, reject) => {
      request(options, (err, res) => {
        ...
        this.serverless.cli.log(
          `Syncing Triggers....Response statuscode: ${res.statusCode}`
        );
        resolve(res);
      });
    });
      ...
      request(options, (err, res, body) => {
        ...
        if (res.statusCode !== 200) {
          return body && body.Error
            ? reject(body.Error)
            : reject("Error executing command, try again later.");
        }
  ...
  uploadPackageJson() {
    ...
    return new BbPromise((resolve, reject) => {
      if (fs.existsSync(packageJsonFilePath)) {
        fs.createReadStream(packageJsonFilePath).pipe(
  ...
  uploadFunction(functionName) {
    return new BbPromise((resolve, reject) => {
      ...
      if (this.serverless.service.functions[functionName].package.artifact) {
        functionZipFile = this.serverless.service.functions[functionName]
          .package.artifact;
      ...
      fs.createReadStream(functionZipFile).pipe(
        request.put(options, (uploadZipErr, uploadZipResponse) => {
          if (uploadZipErr) {
            reject(uploadZipErr);
          } else {
